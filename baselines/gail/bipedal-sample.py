...
class BipedalWalker(gym.Env, EzPickle):
        ...
        for i in range(TERRAIN_LENGTH):
            ...
            if state == GRASS and not oneshot:
                ...
                if i > TERRAIN_STARTPAD:
                    velocity += np_random.uniform(-1, 1) / SCALE  # 1
                y += velocity
    ...
    def _generate_clouds(self):
        ...
        for i in range(TERRAIN_LENGTH // 20):
            x = np_random.uniform(0, TERRAIN_LENGTH) * TERRAIN_STEP
            y = VIEWPORT_H / SCALE * 3 / 4
            poly = [
                (
                    x
                    + 15 * TERRAIN_STEP * math.sin(3.14 * 2 * a / 5)
                    + np_random.uniform(0, 5 * TERRAIN_STEP),
                    y
    ...
    def reset(
        ...
        *,
        ...
        self.hull.ApplyForceToCenter(
            (np_random.uniform(-INITIAL_RANDOM, INITIAL_RANDOM), 0), True
        )
...
if __name__ == "__main__":
    ...
    SPEED = 0.25  # Will fall forward on higher speed
    ...
    while True:
        ...
        if steps % 20 == 0 or terminated or truncated:
            ...
            print("targets " + str([None if x==None else f"{x:+0.2f}" for x in hip_targ + knee_targ]))
        ...
        right_pressed = 0
        left_pressed = 0
        mouse_y = 1 - pygame.mouse.get_pos()[1]/400
        ...
        if hip_targ[moving_leg]:
            hip_targ[moving_leg] = mouse_y*2.+(1-mouse_y)*hip_targ[moving_leg]
        if knee_targ[moving_leg]:
            knee_targ[moving_leg] = mouse_y*-1.+(1-mouse_y)*knee_targ[moving_leg]
        # if left_pressed:
        #     hip_targ[moving_leg] = 1.3
        #     knee_targ[moving_leg] = -.8
        # if right_pressed:
        #     knee_targ[moving_leg] = 0
        #     hip_targ[moving_leg]  = 1


        if hip_targ[0]:
            hip_targ[0] += np_random.normal(0, .2)
            hip_todo[0] = 0.9 * (hip_targ[0] - s[4]) - 0.25 * s[5]
        if hip_targ[1]:
            hip_targ[1] += np_random.normal(0, .2)
            hip_todo[1] = 0.9 * (hip_targ[1] - s[9]) - 0.25 * s[10]
        if knee_targ[0]:
            knee_targ[0] += np_random.normal(0, .2)
            knee_todo[0] = 4.0 * (knee_targ[0] - s[6]) - 0.25 * s[7]
            # knee_todo[0] = (4.0 + mouse_y) * (knee_targ[0] - s[6]) - 0.25 * s[7]
        if knee_targ[1]:
            knee_targ[1] += np_random.normal(0, .2)
            knee_todo[1] = 4.0 * (knee_targ[1] - s[11]) - 0.25 * s[12]
            # knee_todo[1] = (4.0 + mouse_y) * (knee_targ[1] - s[11]) - 0.25 * s[12]
