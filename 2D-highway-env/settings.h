...
namespace SETTINGS {
    ...
    const double PROG_COMPLEXITY_LOSS = 0.05;           // adds L1 loss ( AST.size * PROG_COMPLEXITY_LOSS * loss)
    const double ALPHA_LOSS_UPPER = 0.001;           // adds L2 loss ( alpha^2 * ALPHA_LOSS_UPPER )                        // TEMPERATURE decreases linearly by this much each iteration
}