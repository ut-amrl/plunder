...
void probabilisticProgramTest(){
    ...
    for(int i = 0; i < 1000; i++){
        ...
        if(newHa == stringToHA("ACC")){
            accCount++;
        } else if (newHa == stringToHA("CON")){
            conCount++;
        }
    }
    
    cout << "At position=" << state.pos << ": " << HAToString(ha) << " -> ACC has probability " << accCount/10.0 << "%" << endl;
    cout << "At position=" << state.pos << ": " << HAToString(ha) << " -> CON has probability " << conCount/10.0 << "%" << endl;
}
