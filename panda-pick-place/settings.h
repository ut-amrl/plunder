...
namespace SETTINGS {
    ...
    // ----- EM Loop parameters -----
    const int NUM_ITER = 50;                    // Number of iterations in the expectation-maximization loop
    ...
    const double PROG_COMPLEXITY_LOSS_BASE = 0.03;     // adds L1 loss ( AST.size * PROG_COMPLEXITY_LOSS )
    const double PROG_COMPLEXITY_LOSS = 0.03;          // adds L1 loss ( AST.size * PROG_COMPLEXITY_LOSS * loss )
    const double ALPHA_LOSS_UPPER = 0.0000001;             // adds L2 loss ( alpha^2 * ALPHA_LOSS_UPPER )
}